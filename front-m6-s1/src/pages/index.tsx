...
import { Center, Square, Circle, Button, List, ListItem } from '@chakra-ui/react'
...
const Home = () => {
  ...
  return (
    ...
    <Container height={"100vh"} maxWidth={"100%"}>
      <Box p={10} display={"block"} bg={"red.600"} w={"100%"} borderBottom={"2px"} borderColor={"blackAlpha.500"}>
      {
        isLoged ? 
        <Box bg={"blue.600"}>
          <h2>Seu Nome</h2>
      ...
      <Box justifyContent={"center"} alignItems={"center"} p={10} display={"flex"} bg={"red.600"} w={"100%"} h={"99vh"}>
      {
        isLoged ? 
        <Box display={'flex'} gap={10} flexDirection={"column"} w={"60%"} h={'90%'} p={5} bg={"blue.600"}>
